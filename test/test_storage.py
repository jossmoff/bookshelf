...
from test.utils.storage_utils import (
    create_test_story_dict,
    create_story_json_string,
    create_test_story,
)
...
def test_get_all_stories(bookshelf_storage, mock_home):
    ...
    os.walk.return_value = [
        (BOOKSHELF_TASK_DIR, [], [f"{story.name}.json" for story in test_stories])
    ]
...
def test_get_all_stories_with_invalid_json(bookshelf_storage):
    os.walk = MagicMock()
    os.walk.return_value = [(BOOKSHELF_TASK_DIR, [], ["a.json"])]
    m_open = mock_open()
...
def test_get_all_stories_matching_incomplete_name_filters_out_value(
    ...
    with patch("builtins.open", m_open):
        stories = list(
            bookshelf_storage.get_all_stories_matching_incomplete_name("Test")
        )
